...
const MAX_LOG_ENTRIES: number = 30;
...
export default function KeyboardTest({ isTesting }: { isTesting: boolean }) {
    ...
    const handleKeyDown = useCallback((key_event: KeyboardEvent) => {
        key_event.preventDefault();
        const key = key_event.key.toLowerCase();
        const code = key_event.code.toLowerCase();
    ...
    const handleKeyUp = useCallback((key_event: KeyboardEvent) => {
        const key = key_event.key.toLowerCase();
        const code = key_event.code.toLowerCase();
